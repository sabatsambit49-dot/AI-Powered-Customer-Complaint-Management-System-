        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-lg font-bold text-slate-900 tracking-tight">PharmaQMS</h1>
              <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-700 border border-blue-200">
                API & FDF QA
              </span>
            </div>
            <p className="text-xs text-slate-500">AI-Powered Customer Complaint Intake & Risk Triage System</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 flex-wrap justify-end">
          ...
          <div className="flex items-center bg-slate-100 p-1 rounded-xl border border-slate-200">
            ...
            >
              ...
              {savedCount > 0 && (
                <span className="ml-1 px-1.5 py-0.5 rounded-full text-[10px] bg-blue-50 text-blue-700 font-bold border border-blue-200">
                  {savedCount}
                </span>
              )}
            </button>
          ...
          {user && (
            <div className="flex items-center gap-2 bg-slate-50 pl-3 pr-1.5 py-1.5 rounded-xl border border-slate-200 text-xs">
              <UserCheck className="w-4 h-4 text-blue-600 shrink-0" />
              <div className="leading-tight hidden sm:block">
                <span className="font-semibold text-slate-800 block">{user.full_name}</span>
                <span className="text-[10px] text-slate-500 block">{user.role}</span>
              </div>
              <button
                onClick={() => dispatch(logout())}
                title="Log Out of Portal"
                className="ml-1.5 px-2.5 py-1 rounded-lg bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 font-medium transition flex items-center gap-1"
              >
